 ...
 */
@Deprecated
public class CambriaBatchPublisher implements CambriaBatchingPublisher
{
	 ...
	 */
	public CambriaBatchPublisher ( ConnectionType ct, Collection<String> baseUrls, String topic, int maxBatchSize, long maxAgeMs, boolean compress ) throws MalformedURLException, GeneralSecurityException
	{
		...
		fSender = new Sender ( ct, baseUrls, topic, maxBatchSize, maxAgeMs, compress );
	...
	{
		fSender.setHttpBasicCredentials ( username, password );
	}
	...
	private Logger fLog = LoggerFactory.getLogger ( CambriaBatchPublisher.class );

	private class Sender extends CambriaBaseClient implements Runnable
	{
		public Sender ( ConnectionType ct, Collection<String> baseUrls, String topic, int maxBatch, long maxAgeMs, boolean compress ) throws MalformedURLException, GeneralSecurityException
		{
		...
		public void run ()
		{
			...
			{
				fLog.warn ( "Cambria background send: " + e.getMessage () );
			}
		}
		...
		public void queue ( Collection<message> msgs ) throws IOException
		{
			...
			try
			{
				for ( message userMsg : msgs )
				{
					...
					{
						fLog.warn ( "CambriaBatchPublisher::Sender::queue received a null message." );
					}
				}
			}
			finally
	...
	private static boolean doSend ( LinkedList<TimestampedMessage> toSend, HttpClient client, String topic, boolean compress, Logger log )
	{
		...
		final long nowMs = System.currentTimeMillis ();
		final String url = CambriaConstants.makeUrl ( topic );
		...
		{
			log.warn ( "Problem posting to Cambria: " + e.getMessage() );
		}
		catch ( IOException e )
		{
			log.warn ( "Problem posting to Cambria: " + e.getMessage() );
		...
		log.info ( "cambria response (" + (System.currentTimeMillis ()-startMs) + " ms): OK" );
		return result;
	}
